Step1: A cyber attack occurs.Production is no longer trusted.Nothing inside production can be assumed safe.
...
Step 3: From there.They enter the Core Recovery environment.Think of this as mission control.Nothing sensitive lives here.
Instead it contains the tools needed to recover Automation,Validation,Management,Directory Services,DNS,Logging,Monitoring
...
Step5. Meanwhile Recovery content is coming from two completely different places.This is the most important concept in the whole diagram.
...
Step 6:Nothing gets restored immediately.Everything passes through validation.
This includes: Malware scan --> Integrity verification --> Human approval --> Promotion (Only then is it trusted.)
Step 7: Recovery begins.Infrastructure comes online.Applications starts and Data is restored.Users reconnect and production resumes

Why Three VPCs?Each VPC has one responsibility:
...
Chatper 2-----------------------------------------------
Before we discuss each AWS service, I want you to think of the architecture as layers, not services.
...
Security Hub: The central security dashboard.It aggregates findings from:GuardDuty, Config, Inspector IAM Access Analyzer. Its basically the SOC Dashboard
...
Why in endpoint subnets?Interface endpoints are ENIs(Elastic network interface). They must exist inside a subnet. Remove it and SSM won't work in an isolated environment.
...
Ansible Server / Terraform : Our orchestration is AAP which is a SaaS product which runs on Redhat cloud and then connects to our org using Site-Site VPN and then assumes role in our AWS sandbox account , so why need ansible or terraform here ? 
...
Integrity Validation Compute: Checksum validation.Application testing.Data integrity verification. -- since we are using AAP which performs health check , why need a separate compute ? 
